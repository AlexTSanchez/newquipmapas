...
public class Ayudante extends SQLiteOpenHelper {
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        //Crear tablas temporales, llevar datos a temporales, borrar tablas viejas, crear tablas nuevas, llevar datos temporales, borrar tablas temporales.
        String sql="drop table if exists " + ContratoBaseDatos.TablaNota.TABLA;
        db.execSQL(sql);
        Log.v("sql",sql);
        sql="drop table if exists " + ContratoBaseDatos.TablaNota.TABLA;
        db.execSQL(sql);
        onCreate(db);
    }
}